		...
		if (!Input.GetKey (KeyCode.LeftShift)) {
			...
			foreach (GamepadDevice gamepad in input.gamepads) {
				...
				for (int i = 0; i < buttonValues.Length; i++) {
                    GamepadButton button = (GamepadButton) buttonValues[i];
                    var mapping = gamepad.layout.buttonNames.Find(buttonName => buttonName != null && buttonName.button == button);
                    string name = mapping != null ? mapping.name : "unknown";
                    str += string.Format("{0} ({1}): {2}\n", button, name, gamepad.GetButton((GamepadButton) buttonValues[i]));
				}
